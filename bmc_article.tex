...
\author[
   ...
   email={}   % email address
]{\inits{SAB}\fnm{Simon A} \snm{Bray}}
\author[
   addressref={aff2},
   email={}
]{\inits{TS}\fnm{Tharindu} \snm{Senapathi}}
\author[
   addressref={aff2},
   email={}
]{\inits{CB}\fnm{Christopher} \snm{Barnett}}
\author[
   addressref={aff1,aff2},
   email={john.RS.Smith@cambridge.co.uk}
]{\inits{BG}\fnm{Björn} \snm{Grüning}}
...
\section*{Abstract}
\textbf{TO ADD: why/aim (1st paragraph of the introduction)}. This paper
is a tutorial developed for the data analysis platform Galaxy. The
Galaxy's concept makes high-throughput sequencing data analysis a
...
In this tutorial we focus on 3 questions: How are protein-ligand systems
parameterized for molecular dynamics simulation? What kind of analysis
can be carried out on molecular trajectories? How can high-throughput MD
be used to study multiple ligands? After finishing this tutorial you
will be able to learn about force-fields and MD parameterization, learn
how to conduct MD simulation and analysis for a protein-ligand system,
understand how different molecular interactions contribute to the
binding affinity of various ligands for the Hsp90 protein..
...
\cite{afgan2018galaxy} is a data analysis platform that provides access
...
Training Network \cite{batut2018community} and its most up-to-date
version is available online on the
\href{https://training.galaxyproject.org/topics/computational-chemistry/tutorials/hthtml-analysis_local/tutorial.html}{Galaxy
...
\item
  ...
  is correct (PDB file). \textgreater{} \textgreater{} \{: .hands\_on\}
\end{enumerate}
...
\item
  ...
  \texttt{System\ GRO\ file}. \{: .hands\_on\}
\end{enumerate}
\end{quote}
...
\item
  ...
  \texttt{System\ topology}. \{: .hands\_on\}
\end{enumerate}
\end{quote}
...
\begin{figure}
\centering
\includegraphics{images/workflow_htmd_analysis.png}
\caption{Analysis workflow for Hsp90 with ligand}
\end{figure}
...
\hypertarget{rmsd-analysis---protein}{%
\subsection{RMSD analysis - protein}\label{rmsd-analysis---protein}}
...
RMSD of the Cα atoms of the protein backbone is calculated here is and
is a measire of how much the protein has changed between different time
...
\caption{RMSD timeseries Hsp90}
\end{figure}

\begin{figure}
\centering
\includegraphics{images/htmd_analysis_rmsd1_histo.png}
\caption{RMSD histogram Hsp90}
\end{figure}

RMSD timeseries shows a thermally stable and equilibrated structure with
an average RMSD between 1 and 1.3. There are no large conformational
changes during the simulation. The RMSD histogram confirms this.

\hypertarget{rmsd-analysis---ligand}{%
\subsection{RMSD analysis - ligand}\label{rmsd-analysis---ligand}}

Calculating the RMSD of the ligand is necessary to check if it is stable
in the active site and to identidfy possible binding modes. If the
ligand is not stable the RMSD will fluctuate excessively and there will
large `jumps' in the RMSD.

Note two RMSD's are run but only one is considered further. The tool has
a ligand inference option but this may fail and instead the ligand is
selected by residue ID.
...
The RMSD seems to stabilise at 1.0 and then flips back to 0.5. This is
more clearly seen in the histogram. There is a bimodal distribution with
centres at 0.3 and 1.0 which both have significant populations. There
are two binding poses. These can be visually assessed. In this case the
poses were viewed in VMD.

\begin{figure}
\centering
\includegraphics{images/htmd_analysis_rmsd2_series.png}
\caption{RMSD timeseries Hsp90 ligand}
\end{figure}

\begin{figure}
\centering
\includegraphics{images/htmd_analysis_rmsd2_histo.png}
\caption{RMSD histogram Hsp90 ligand}
\end{figure}

\begin{figure}
\centering
\includegraphics{images/htmd_analysis_bindingposes.png}
\caption{Hsp90 ligand binding poses}
\end{figure}
...
*** TODO *** gif of PCA1 \includegraphics{images/image_name}
\includegraphics{images/htmd_analysis_pc1_hsp90.gif}
...
bond is seen between the ligand andHsp90, this is between Threonine 184
...
\section{Optional: Automating high throughput
...
paramters, data and outputs quite well. Junk in will mean junk out. The
...
manaual task and then run a workflow. Realise if there is a manual task,
...
Sum up the tutorial and the key takeaways here. We encourage adding an
overview image of the pipeline used.
