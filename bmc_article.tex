...
3D Cartesian coordinates of each atom. Therefore, before beginning simulation, we need to calculate
the rest of the information contained within the topology file. There
are a range of force fields to perform these calculations.
...
Here, we use GAFF (general AMBER force field), which is a generalized AMBER force field which can be applied to almost any small organic molecule, not just macromolecules such as proteins.
...
format. This file will be used by most analysis tools as a starting structure. This tool can also be used to do initial setup for GROMACS simulations and convert from PDB to GRO format
...
points in the trajectory. An RMSD on C$\alpha$ atoms will select all protein C$\alpha$ atoms, where the ligans is also a protein consider a different selection. 
...
using Molecular Mechanics Poisson-Boltzman or General Born Surface Area (MMPBSA or MMGBSA). For
...
AMBER's parmed to carry out the conversion. The result will be 4 AMBER'.prmtop'
