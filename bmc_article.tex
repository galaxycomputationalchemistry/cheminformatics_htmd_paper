...
is a measure of how much the protein conformation has changed between different time points in the trajectory. Note that if protein-protein interactions were being studied, we would need to use a more focused selection to distinguish the C$\alpha$ atoms of the different chains.

For the RMSD analysis of the ligand, the `Select domains' parameter of the tool can for convenience be set to `Ligand'; however, this automatic selection sometimes fails. The other alternative, which we apply here, is tp specify the `Residue ID' in the textbox provided. In this example the ligand's Residue ID is `G5E'. The output is the requested RMSD data as a time series, the RMSD plotted as a time series and as a histogram (for example, see Figure \ref{fig:rmsdprotein} in the results section).
...
\subsection*{RMSF}
When considering the RMSF (Figure \ref{fig:rmsf}), fluctuations greater than 1.0{\AA} are of interest, for example see the fluctuations near residue positions 50, 110 and 160. These are actually residues 67, 127 and 167 as the first few residues of this protein are missing in the PDB and were not added using homology modeling in this tutorial. Specifically asp-66 pro-67 ser-68; ala-126 asp-127 ile-128; thr-175 thr-176 glu-178. Residue position 0 in the RMSF is actually position 17 in the Hsp90 FASTA primary sequence. This is a fairly common problem that can occur with molecular modeling of proteins where there may be missing residues at the beginning or within the sequence. It is important to double check and often the residue names are given for clarity. Very large fluctuations are seen for the C-terminus; this is common and no investigation is needed. The overall movement seen in the RMSF could be related to binding or other motions.
...
\begin{figure}[ht!]
  ...
  \caption{\csentence{RMSF for the protein.}
      RMSF(\AA) vs the residue position. Large fluctuations occur for occur at various positions, which correspond to flexible loop regions on the surface of the protein.}
\label{fig:rmsf}
\end{figure}
...
\item
  ...
  \end{itemize}
\item
  Import the analysis workflow from the European~\cite{eu_htmd_analysis_workflow} or the South African Galaxy server~\cite{za_htmd_analysis_workflow}.
\item
  Run the imported workflow with the following parameters:
  \begin{itemize}
  \tightlist
