...
For the RMSD analysis of ligand, the `Select domains' parameter of the tool can be set to `Ligand', however this automatic selection sometimes fails. Instead, the selection is done with the `Residue ID' which is then specified in the textbox provided. In this example the ligand's Residue ID is `G5E'. The output is the requested RMSD data as a time series, the RMSD plotted as a time series and as a histogram (for example see Figure \ref{fig:rmsdprotein} in the results section).
